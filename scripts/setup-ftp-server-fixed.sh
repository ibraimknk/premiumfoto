...
# FTP sunucusu kurulumu ve yapılandırması (apt update hatası düzeltilmiş)
...
if ! command -v vsftpd &> /dev/null; then
    ...
    # Apt update hatasını atla
    sudo apt update 2>&1 | grep -v "packagecloud.io" || true
...
if [ -z "$FTP_PASSWORD" ]; then
    ...
    echo -e "${YELLOW}📝 Şifre: ${FTP_PASSWORD}${NC}"
else
    echo "$FTP_USER:$FTP_PASSWORD" | sudo chpasswd
