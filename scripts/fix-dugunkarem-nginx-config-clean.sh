...
# dugunkarem.com ve dugunkarem.com.tr için temiz Nginx config oluşturma
...
echo -e "${BLUE}🔧 dugunkarem.com Nginx config temizleniyor...${NC}"
echo ""
...
# Python script ile temizle ve düzelt
echo -e "${YELLOW}🔧 Nginx config temizleniyor...${NC}"
sudo python3 << PYEOF
import re
import sys
...
try:
    ...
    # Bozuk satırları temizle (return 301 https://; gibi)
    content = re.sub(r'return\s+301\s+https://\s*;', 'return 301 https://$host$request_uri;', content)
    ...
    # $ karakterlerini escape etmek için string concatenation kullan
    dollar = "$"
    dugunkarem_blocks = f'''
