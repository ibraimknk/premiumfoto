...
# Nginx config'i sıfırdan yeniden oluştur (daha dikkatli)
...
# 2. Mevcut config'den sadece fotougur.com.tr block'larını düzgün bir şekilde al
    ...
    if in_server_block:
        ...
        if brace_count == 0:
            ...
            if 'fotougur.com.tr' in block_content.lower() and 'dugunkarem' not in block_content.lower():
                fotougur_blocks.append(block_content)
            in_server_block = False
            server_block_lines = []
...
# Mevcut config'den fotougur.com.tr block'larını düzgün bir şekilde al
with open(config_file, 'r', encoding='utf-8') as f:
    lines = f.readlines()
...
if sudo nginx -t; then
    ...
    echo ""
    echo -e "${YELLOW}💡 Satır 90-100:${NC}"
    sudo sed -n '90,100p' "$NGINX_CONFIG"
    echo ""
